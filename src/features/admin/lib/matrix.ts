...
/* ===========================================================================
 * 4. ONE OPERATOR — the same visual, for a single person.
 *
 * The Operators tab shows the matrix for the person on screen, so the header
 * climbs from the top of their branch down to the LEVEL WHERE THEY WORK (their
 * own node) — the maintainer's rule, 2 September: "the hierarchy level should
 * go the lowest in this one where the operator works". There is no company-wide
 * span to draw here because there is only one branch in view: theirs.
 * ======================================================================== */

export interface OperatorMatrixInput {
  nodes: readonly BoardNode[];
  levels: readonly HierarchyLevel[];
  skills: readonly SkillRecord[];
  operatorSkills: readonly OperatorSkillRecord[];
  /** The person on screen — only `id` and `siteNodeId` are load-bearing. */
  operator: Pick<OperatorRecord, "id" | "siteNodeId">;
  today: string;
  windowDays: number;
  includeInactive?: boolean;
}

export interface OperatorMatrixModel {
  columns: MatrixColumns;
  cellState: (skillId: string) => CellState;
  counts: { trainings: number; held: number; gaps: number; needRenewal: number };
}

/**
 * Build the single-operator matrix.
 *
 * Columns are the trainings that APPLY to this person (owner ancestor-or-self —
 * the §19.72 path test), PLUS any they already hold. The second half matters:
 * a grant made before somebody moved off a branch is still theirs to see and to
 * remove, and this is the only screen that can revoke it, so a held training is
 * never dropped for no longer applying.
 *
 * ⚠️ A HELD TRAINING READS AS HELD even where it no longer applies — its expiry
 * still decides trained / expiring / expired, never `na`. Only an UNHELD column
 * leans on the apply test, and it is there precisely because the training
 * applies (or it would not be a column at all), so it shows the `missing` gap
 * the supervisor can fill. `na` is therefore unreachable here, which is the
 * point: on one person's own screen, every training shown is one that is theirs
 * to act on.
 */
export function buildOperatorMatrix(input: OperatorMatrixInput): OperatorMatrixModel {
  const byId = new Map(input.nodes.map((n) => [n.id, n] as const));
  const levelsById = new Map(input.levels.map((l) => [l.id, l] as const));
  const includeInactive = input.includeInactive ?? false;
  const opPath = byId.get(input.operator.siteNodeId)?.path;

  const holdings = new Map<string, OperatorSkillRecord>();
  for (const h of input.operatorSkills) {
    if (h.operatorId === input.operator.id) holdings.set(h.skillId, h);
  }

  const applies = (skill: SkillRecord): boolean => {
    const ownerPath = byId.get(skill.siteNodeId)?.path;
    return ownerPath !== undefined && opPath !== undefined && trainingApplies(ownerPath, opPath);
  };

  const skills = input.skills.filter((s) => {
    if (!includeInactive && !s.active) return false;
    return holdings.has(s.id) || applies(s);
  });

  const columns = buildColumns(skills, byId, levelsById);
  const skillById = new Map(skills.map((s) => [s.id, s] as const));

  const cellState = (skillId: string): CellState => {
    const skill = skillById.get(skillId);
    if (skill === undefined) return "na";
    const holding = holdings.get(skillId);
    return cellStateFor(holding, holding !== undefined || applies(skill), input.today, input.windowDays);
  };
  ...
  let needRenewal = 0;
  for (const s of columns.cols) {
    const st = cellState(s.id);
    if (holdings.has(s.id)) held++;
    if (st === "missing") gaps++;
    else if (st === "expiring" || st === "expired") needRenewal++;
  }
  ...
  return {
    columns,
    cellState,
    counts: { trainings: columns.cols.length, held, gaps, needRenewal },
  };
}
