...
/* ⚠️⚠️ THE `ownedInScope` CASES (XO1-XO4) WERE HERE AND ARE DELETED WITH THE
   FUNCTION (S39 / R-346). They asserted the right thing about the wrong
   question: that a person whose owner is not one of the board's nodes is "not
   this plant". `board_window` sends the nodes at or below the READER'S root, so
   for a supervisor granted a line that predicate dropped everyone homed at the
   plant above her and her panel was empty. The pool is the server's answer now
   (migration 0058 sends the plant's people), and who is offered WHERE is
   `splitPeopleFor` in `features/board/lib/outsideArea.ts`, over paths rather
   than over a map whose narrowing the client cannot name. The cases moved with
   the question: `src/test/outsideArea.test.ts`. */
