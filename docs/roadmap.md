...
**Last updated:** 2026-08-25 (**P1-5a AND P1-5b built, independently verified, and FULL ACCEPTANCE GREEN.** typecheck/lint/build clean, **236 unit tests pass** (was 135). P1-5b's agent cost **243k** vs P1-5a's 329k. **D78 (§19.12): the brief specified a 76-assertion suite but never named a repo path for it, so the agent's suite died with its container.** The design session wrote `src/test/hierarchy.test.ts` — **101 assertions, 17/17 mutations caught**. **Three defects fixed, none of them the agent's logic:** `validateLevelDraft` THREW on a null/missing name where the server returns `blank_name`; `database.types.ts` was 3 days stale so all five new RPCs were untyped (**a migration and its generated types are ONE change**); and `p_parent_id`/`p_new_parent_id` needed the `mutations.ts` null-cast, because Postgres parameters carry no nullability and `gen types` can never emit `string | null`. **Owed to the server before P1-5d:** Postgres `trim()` strips spaces only, so it ACCEPTS a tab-only level name the client rejects (§19.12). **P1-5 is four briefs**: 5a database ✓, 5b client layer ✓, 5c admin screens, 5d CSV import.) · **Current phase:** 1 — Core product
...
- [ ] Org onboarding settings pages (same pattern as ⚙ Shifts): hierarchy level editor + node tree editor + CSV import (operators, products, tree) — **re-split into four (§19.9): P1-5a database half — built + validated + mutation-tested; P1-5b client layer — built, Part A verified (agent 80/80, design session 101/101 committed); P1-5c admin screens; P1-5d CSV import**. The split got finer because P1-4c cost 69k against P1-4b's 336k purely on tightness
...
> `docs/design-plan.md` §19 (especially §19.11 and §19.12), and pick up at the resume point recorded in
> memory. The local Supabase stack is already running.
>
> Context you won't get from the docs: P1-5b is built and independently verified, and I have run the
> npm acceptance since — check `checks.txt` for the result before trusting anything, and **check
> `git status`**, since the Aug 25 work may or may not be committed.
>
> The next action is to write the P1-5c brief (admin screens). Read the brief-writing rules and the
> verification standard in memory first. Note D78 in §19.12: a brief that specifies a test suite must
> name the suite's repo path in its file table, or the suite dies with the agent's container — that is
> exactly what happened to P1-5b. §19.12 also lists four things P1-5c specifically owes.
