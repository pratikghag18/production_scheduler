...
**Last updated:** 2026-08-25 (**P1-5a, P1-5b and P1-5c all built and independently verified.** P1-5c closes the three board debts: the §19.11 popover inline-width defect plus committed regression tests for §19.6 and §19.8, neither of which had been guarded by anything. **D80 (§19.13): `verify-db.sh` had been creating the scratch DB as SQL_ASCII, not UTF-8** — every SQL test this project ever ran used a database configured unlike Supabase, and every one passed. Fixed, and the encoding is now asserted. **D81 (§19.14): quoting an artifact is not enough — EVALUATE it across the range you actually run in.** `--chrome-scale` is pinned flat at 1.0 below 1440px, so the popover's `ResizeObserver` never fires on an ordinary laptop and its captured viewport went stale. **D82 (§19.14): heredoc delivery is right for NEW files and wrong for EDITS** — P1-5c cost **370k** against P1-5b's 243k for half the assertions, because it edited 8 existing files instead of creating 5 new ones. **⚠️ npm acceptance NOT re-run since P1-5c, and migration 0011 is not yet applied to the local Supabase.** **P1-5 is five briefs**: 5a ✓, 5b ✓, 5c ✓, 5d admin screens, 5e CSV import.) · **Current phase:** 1 — Core product
...
- [ ] Org onboarding settings pages (same pattern as ⚙ Shifts): hierarchy level editor + node tree editor + CSV import (operators, products, tree) — **re-split into four (§19.9) then five (§19.13): P1-5a database half ✓; P1-5b client layer ✓ (agent 80/80, design session 101/101 committed); P1-5c board debts; P1-5d admin screens; P1-5e CSV import**. The split got finer because P1-4c cost 69k against P1-4b's 336k purely on tightness
...
> `docs/design-plan.md` §19 (especially §19.12, §19.13 and §19.14), and pick up at the resume point
> recorded in memory. The local Supabase stack is already running.
>
> Context you won't get from the docs: P1-5a, 5b and 5c are all built and verified, but **the npm
> acceptance has not been run since P1-5c** and **migration 0011 is not yet applied to my local
> Supabase** — I need to run `supabase db reset` in WSL. **Check `git status`**, since none of this
> may be committed yet.
>
> The next action is to write the P1-5d brief (the hierarchy admin screens). Read the brief-writing
> rules and the verification standard in memory first — rules 11, 12 and 13 are all new and all came
> out of the last three briefs.
...
### Build output (Aug 25, after P1-5b)

| chunk | raw | gzipped |
|---|---|---|
| app (`index`) | 98.96 kB | 29.77 kB |
| query | 50.11 kB | 15.40 kB |
| router | 94.33 kB | 31.81 kB |
| react | 180.98 kB | 56.86 kB |
| supabase | 219.90 kB | 57.42 kB |
| **total JS** | **644.28 kB** | **191.26 kB** |
| CSS | 25.72 kB | 4.58 kB |

P1-5b added **1.5 kB raw** to the app chunk — the whole client layer. The Aug 24 → Aug 25 jump
(40 → 97 kB) was P1-4b–e; nothing since has moved the needle.

**The honest read is unchanged:** `manualChunks` buys caching granularity, not deferral — every chunk
is still imported by the entry, so this is all first-load. **Route-level `React.lazy` is the actual
fix** and is still unwritten. `/admin` grows substantially in P1-5c, which makes it the natural first
candidate to split.
...
| P1-5d | *(not yet written)* | The admin screens: hierarchy level editor + node tree editor, on P1-5b's pure lib and typed wrappers | **OWES (§19.12):** confirm or override the `hierarchyKeys.all = ["hierarchy"]` invalidation key P1-5b introduced for a read hook that does not exist yet; **always pass `canDropOn` the COMPLETE level array** (a partial one makes the client reject a move the server accepts); and do not assume `create_node`'s `level_mismatch` `node_id` identifies the subject of the error — it carries the PARENT's id |
| P1-5e | *(not yet written)* | CSV import: a pure RFC 4180 parse/plan module (quoting, BOM, CRLF, header mapping, duplicate detection, parents-before-children ordering) + the import wizard + upsert-by-`external_id`. §7 calls this "the sync pipeline with a manual trigger", so it validates the whole upsert machinery | **BLOCKED ON:** a second seeded org — cross-org isolation is untested by anything today, so a missing `org_id` filter passes every test in the repo. Add it before the upsert lands. Note `app_trim_ws` already strips U+FEFF, so a BOM in a header cell will not survive into a name |
