...
**Last updated:** 2026-08-25 (**P1-5a built and verified.** Migration 0010 closes six measured schema holes incl. a subtree-grant leak. Design-session verification ran an independent 36-case probe plus six *unprescribed* mutations — none of which either suite caught — and a NULL-argument sweep that found **three real defects, two of them bugs in my own brief**, worst being `delete_node(id, NULL)` silently hard-deleting. All fixed; suite now 43 cases. Next: **P1-5b, the admin pages.** See design plan §19.5.) · **Current phase:** 1 — Core product
...
- [ ] Org onboarding settings pages (same pattern as ⚙ Shifts): hierarchy level editor + node tree editor + CSV import (operators, products, tree) — **split into P1-5a (database half — built + validated + mutation-tested, Aug 25), P1-5b (admin pages), P1-5c (CSV import)**; see design plan §19.4
