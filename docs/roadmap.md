...
**Last updated:** 2026-08-25 (**P1-5a/5b/5c built + verified, FULL ACCEPTANCE GREEN — 283 unit tests, typecheck/lint/build clean.** Then a **second org was seeded, and it found a real cross-tenant READ AND WRITE LEAK within ten minutes (§19.15, D83).** `app_can_read_node`/`app_can_edit_node` are SECURITY DEFINER and tested ltree containment ONLY — no org scope — so an org-1 admin saw another tenant's runs and assignments, and a non-admin supervisor **successfully UPDATED another tenant's run**. All eight `runs`/`assignments` policies delegate entirely to those two functions, so the leak lived exactly where the schedule does. **Migration 0012** fixes both functions plus a latent org-blind path-rewrite in `nodes_cascade_path()`. **`supabase/tests/80_cross_org_test.sql` — 20 cases, ALL of which failed before 0012**; 3/3 mutations caught by named cases. **⚠️ 0012 and the new seed are NOT yet applied to the local Supabase, and the npm acceptance has not been re-run since.** **P1-5 is five briefs**: 5a ✓, 5b ✓, 5c ✓, 5d admin screens, 5e CSV import — **5e is now UNBLOCKED**.) · **Current phase:** 1 — Core product
